...
### 3. 数据类型 (Data Types) - 详解

选择合适的数据类型对数据库性能至关重要。原则：**在满足需求的前提下，尽量使用更小的类型**。

#### 3.1 数值类型 (Numeric)

| 类型             | 字节 | 范围 (Signed)  | 说明              | 使用场景                  |
| :--------------- | :--- | :------------- | :---------------- | :------------------------ |
| **TINYINT**      | 1    | -128 ~ 127     | 小整数            | 年龄、状态码 (0/1)        |
| **INT**          | 4    | -21 亿 ~ 21 亿 | 标准整数          | 用户 ID、大多数整数       |
| **BIGINT**       | 8    | 极大           | 大整数            | 推特量级的 ID、金融流水号 |
| **FLOAT**        | 4    | 精度有限       | 单精度浮点        | 科学计算 (不推荐用于金额) |
| **DOUBLE**       | 8    | 精度更高       | 双精度浮点        | 科学计算 (不推荐用于金额) |
| **DECIMAL(M,D)** | M+2  | 依赖定义       | **定点数 (精准)** | **金额、财务数据**        |

> **实战技巧**:
>
> - 定义无符号整数使用 `UNSIGNED`，例如 `TINYINT UNSIGNED` 范围变为 0~255。
> - 金额永远不要用 FLOAT/DOUBLE，必须用 `DECIMAL`，例如 `DECIMAL(10, 2)` 表示总长 10 位，小数 2 位。

#### 3.2 字符串类型 (String)

| 类型           | 长度特点     | 最大长度   | 说明                          | 使用场景                        |
| :------------- | :----------- | :--------- | :---------------------------- | :------------------------------ |
| **CHAR(N)**    | **固定长度** | 255 字符   | 速度快，不足补空格            | 身份证、手机号、哈希值 (MD5)    |
| **VARCHAR(N)** | **可变长度** | 65535 字节 | 节省空间，额外 1-2 字节存长度 | 用户名、地址、评论              |
| **TEXT**       | 可变长       | 64KB       | 长文本                        | 文章内容、备注 (不走内存临时表) |
| **LONGTEXT**   | 可变长       | 4GB        | 极大文本                      | 存储整本书                      |

> **实战技巧**:
>
> - `VARCHAR(N)` 中的 N 是**字符数**不是字节数 (utf8mb4 下一个字符占 4 字节)。
> - 尽量不用 `TEXT/BLOB`，会引起全表扫描时的磁盘临时表，影响性能。如果必须用，建议拆分单独的表存储。

#### 3.3 日期与时间 (Date & Time)

| 类型          | 格式                | 范围      | 时区无关?            | 说明                     |
| :------------ | :------------------ | :-------- | :------------------- | :----------------------- |
| **DATE**      | YYYY-MM-DD          | 1000~9999 | 是                   | 只有日期                 |
| **DATETIME**  | YYYY-MM-DD HH:mm:ss | 1000~9999 | **是** (原样存取)    | **最常用**，记录发生时间 |
| **TIMESTAMP** | YYYY-MM-DD HH:mm:ss | 1970~2038 | **否** (转 UTC 存储) | 适合记录跨时区业务       |

#### 3.4 JSON 类型 (8.0+)

MySQL 5.7+ 开始支持，8.0 极大增强。

- 存储结构化数据，自动校验 JSON 格式。
- 支持索引 (通过虚拟列生成)。
- **场景**: 用户配置信息、不确定的扩展属性。

#### 3.5 🔥 核心辨析：存储宽度 vs 显示宽度 (面试高频)

这是 MySQL 中最大的误区之一，**整数**和**字符串**中的括号 `(N)` 含义完全不同！

**1. 整数类型的 N (显示宽度)**
例如 `INT(1)` vs `INT(11)`：

- **误区**: 以为 `INT(1)` 改为了只占 1 个字节，或者只能存 0-9。
- **真相**: **没有任何区别！**
  - **存储空间**: 都是 4 字节。
  - **取值范围**: 都是 -21 亿 ~ 21 亿。
  - **N 的作用**: 仅仅是**显示宽度 (Display Width)**。只有配合 `ZEROFILL` 属性时才有意义（不足 N 位补前导零）。例如 `INT(3) ZEROFILL`，插入 `1` 会显示 `001`。
- **注意**: MySQL 8.0.17+ 已经废弃了整数类型的显示宽度属性，建议直接写 `INT`。

**2. 字符串类型的 N (存储限制)**
例如 `VARCHAR(10)`：

- **含义**: 这里 N 代表**最大字符数**（不是字节数）。
- **限制**: 这是一个硬限制。如果你定义 `VARCHAR(10)`，试图存入 11 个字符会报错。
- **中文**: 在 `utf8mb4` 编码下，`VARCHAR(10)` 能存 10 个汉字（尽管它实际上占用了 30~40 个字节）。
...
CREATE TABLE user_profile (
    ...
    -- 数值练习
    age TINYINT UNSIGNED DEFAULT 18 COMMENT '年龄(0-255)',
    balance DECIMAL(15, 2) DEFAULT 0.00 COMMENT '余额(精准)',

    -- 字符串练习
    country_code CHAR(3) COMMENT '国家代码(固定3位,如CHN)',
    username VARCHAR(50) NOT NULL COMMENT '用户名',
    bio VARCHAR(200) COMMENT '简介',

    -- 状态(推荐用 TINYINT 替代 ENUM)
